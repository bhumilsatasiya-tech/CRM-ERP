...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('export_invoices', function (Blueprint $table) {
            ...
            $table->enum('incoterm', ['FOB', 'CIF', 'EXW', 'CFR', 'DAP', 'DDP'])->default('FOB');
            $table->string('port_of_loading', 128)->nullable();
            $table->string('port_of_discharge', 128)->nullable();
            $table->string('country_of_destination', 64)->nullable();
            ...
            $table->unique(['company_id', 'code'], 'export_invoices_company_code_unique');
            $table->index(['company_id', 'status', 'deleted_at']);
            $table->index('invoice_date');
        });
    }

    public function down(): void { Schema::dropIfExists('export_invoices'); }
};
