...
class StoreExportInvoiceRequest extends FormRequest
{
    public function authorize(): bool { return $this->user()?->can('export.invoice.create') ?? false; }

    public function rules(): array
    {
        $companyId = app()->bound('active_company_id') ? app('active_company_id') : null;
        $eiId = $this->route('export_invoice')?->id;
        return [
            'code'                   => ['nullable', 'string', 'max:64', Rule::unique('export_invoices', 'code')->ignore($eiId)->where(fn($q) => $companyId ? $q->where('company_id', $companyId) : $q)->whereNull('deleted_at')],
            ...
            'country_of_destination' => ['nullable', 'string', 'max:64'],
